...
//recent calls
				...
				document.getElementById('recent_calls_chart').getContext('2d'),
				{
					...
					data: {
						datasets: [{
							...
							backgroundColor: [
								'<?php echo ($settings->get('theme', 'dashboard_recent_calls_chart_main_color') ?? '#007aff'); ?>',
								'<?php echo ($settings->get('theme', 'dashboard_recent_calls_chart_sub_color') ?? '#d1d1d6'); ?>'
							],
	...
	if (!isset($widget_chart_type) || $widget_chart_type == "icon") {
		...
		echo "		<span style=\"background-color: ".(!empty($widget_number_background_color) ? $widget_number_background_color : '#007aff')."; color: ".(!empty($widget_number_text_color) ? $widget_number_text_color : '#ffffff')."; font-size: 10px; text-align: center; position: absolute; top: 28px; left: 24px; padding: 2px 7px 1px 7px; border-radius: 10px; white-space: nowrap;\">".$num_rows."</span>\n";
		echo "	</div>\n";
	}
