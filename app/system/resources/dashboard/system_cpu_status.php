	...
	if ($widget_chart_type === 'line') { ?>
		...
		<script>
			...
			const dashboard_cpu_usage_chart_main_color = [
				'<?php echo ($settings->get('theme', 'dashboard_cpu_usage_chart_main_color')[0] ?? '#34c759'); ?>',
				'<?php echo ($settings->get('theme', 'dashboard_cpu_usage_chart_main_color')[1] ?? '#ff9500'); ?>',
				'<?php echo ($settings->get('theme', 'dashboard_cpu_usage_chart_main_color')[2] ?? '#ff3b30'); ?>'
			];
			...
			const base_colors = ['#007aff', '#34c759', '#ff9500', '#ff3b30', '#af52de', '#00c7be', '#32ade6', '#5856d6'];
	...
	if (!isset($widget_chart_type) || $widget_chart_type == "doughnut") { ?>
		...
		<script>
			...
			function update_cpu_chart(payload) {
				...
				if (cpu_rounded <= 60) {
					chart.data.datasets[0].backgroundColor[0] = '<?php echo ($settings->get('theme', 'dashboard_cpu_usage_chart_main_color')[0] ?? '#34c759'); ?>';
				} else if (cpu_rounded <= 80) {
					chart.data.datasets[0].backgroundColor[0] = '<?php echo ($settings->get('theme', 'dashboard_cpu_usage_chart_main_color')[1] ?? '#ff9500'); ?>';
				} else {
					chart.data.datasets[0].backgroundColor[0] = '<?php echo ($settings->get('theme', 'dashboard_cpu_usage_chart_main_color')[2] ?? '#ff3b30'); ?>';
				}
					...
					data: {
						datasets: [{
							...
							backgroundColor: [
								<?php
								if ($percent_cpu <= 60) {
									echo "'".($settings->get('theme', 'dashboard_cpu_usage_chart_main_color')[0] ?? '#34c759')."',\n";
								} else if ($percent_cpu <= 80) {
									echo "'".($settings->get('theme', 'dashboard_cpu_usage_chart_main_color')[1] ?? '#ff9500')."',\n";
								} else if ($percent_cpu > 80) {
									echo "'".($settings->get('theme', 'dashboard_cpu_usage_chart_main_color')[2] ?? '#ff3b30')."',\n";
								}
								?>
								'<?php echo ($settings->get('theme', 'dashboard_cpu_usage_chart_sub_color') ?? '#d1d1d6'); ?>'
							],
